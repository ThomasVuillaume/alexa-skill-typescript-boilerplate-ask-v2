...
function Post-Install-Hook() {
    <#
        .SYNOPSIS
            Function to run the `post-install-hook` script declared in package.json
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param()
    ...
    begin {
        Show-Log "Running Post-Install-Hook now."
    }
    process {
        $DepCmd = "npm run post-install-hook"
        Invoke-Expression -Command $DepCmd
        if(!($LASTEXITCODE -eq 0)) {
            Show-Log "post-install-hook exited with an error." "Error"
        }
        return $true
    }
    end {}
...
if (Post-Install-Hook) {
    Show-Log "Post-Install-Hook success."
}
