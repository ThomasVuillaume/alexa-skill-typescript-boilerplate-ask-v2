...
main() {
  ...
  if ! install_dependencies; then
    ...
    if ! run_post_install_hook; then
      display_stderr "The post-install-hook script failed."
      exit 1
    else
...
run_post_install_hook() {
  [[ $DO_DEBUG == true ]] && display_debug "Running Post-Install-Hook now, as declared in package.json" 

  npm run post-install-hook
  return $?
}
